...
//Qual jogo está ativo no momento
typedef enum {
    NONE = 0,
    CAPIVARAIMPOSTORA = 1,
    DESVIECAPIVARA = 2,
...
// Aqui executamos as operações de renderização
        ...
        // Mas a capivara são 4 objetos dferentes, então aqui tem os números do modelo total
...
// Aqui executamos as operaÃ§Ãµes de renderizaÃ§Ã£o
        ...
        // Computamos a posiÃ§Ã£o da cÃ¢mera utilizando coordenadas esfÃ©ricas.  As
        // variÃ¡veis g_CameraDistance, g_CameraPhi, e g_CameraTheta sÃ£o
        // controladas pelo mouse do usuÃ¡rio. Veja as funÃ§Ãµes CursorPosCallback()
        // e ScrollCallback().
        float r = g_CameraDistance;
        float y = r*sin(g_CameraPhi);
        float z = r*cos(g_CameraPhi)*cos(g_CameraTheta);
        ...
        // Abaixo definimos as varÃ¡veis que efetivamente definem a cÃ¢mera virtual.
        // Veja slides 195-227 e 229-234 do documento Aula_08_Sistemas_de_Coordenadas.pdf.
        //camera_position_c  = glm::vec4(0.0f,3.0f,-1.0f,1.0f); // Ponto "c", centro da cÃ¢mera
        //glm::vec4 camera_lookat_l    = glm::vec4(0.0f,-10.0f,1.0f,1.0f); // Ponto "l", para onde a cÃ¢mera (look-at) estarÃ¡ sempre olhando
        glm::vec4 camera_view = glm::vec4(x,y,z,0.0f); // Vetor "view", sentido para onde a cÃ¢mera estÃ¡ virada
        glm::vec4 camera_up_vector   = glm::vec4(0.0f,1.0f,0.0f,0.0f); // Vetor "up" fixado para apontar para o "cÃ©u" (eito Y global)

        // Computamos a matriz "View" utilizando os parÃ¢metros da cÃ¢mera para
        // definir o sistema de coordenadas da cÃ¢mera.  Veja slides 2-14, 184-190 e 236-242 do documento Aula_08_Sistemas_de_Coordenadas.pdf.
        glm::mat4 view = Matrix_Camera_View(variaveis->camera_position_c, camera_view, camera_up_vector);

        // Agora computamos a matriz de ProjeÃ§Ã£o.
        glm::mat4 projection;

        // Note que, no sistema de coordenadas da cÃ¢mera, os planos near e far
        // estÃ£o no sentido negativo! Veja slides 176-204 do documento Aula_09_Projecoes.pdf.
        float nearplane = -0.1f;  // PosiÃ§Ã£o do "near plane"
        float farplane  = -10.0f; // PosiÃ§Ã£o do "far plane"

        if (g_UsePerspectiveProjection)
        {
            // ProjeÃ§Ã£o Perspectiva.
            // Para definiÃ§Ã£o do field of view (FOV), veja slides 205-215 do documento Aula_09_Projecoes.pdf.
            float field_of_view = 3.141592 / 3.0f;
            projection = Matrix_Perspective(field_of_view, g_ScreenRatio, nearplane, farplane);
        }
        else
        {
            ...
            // Para definiÃ§Ã£o dos valores l, r, b, t ("left", "right", "bottom", "top"),
            // PARA PROJEÃ‡ÃƒO ORTOGRÃFICA veja slides 219-224 do documento Aula_09_Projecoes.pdf.
            // Para simular um "zoom" ortogrÃ¡fico, computamos o valor de "t"
            // utilizando a variÃ¡vel g_CameraDistance.
            float t = 1.5f*g_CameraDistance/2.5f;
            float b = -t;
            float r = t*g_ScreenRatio;
            float l = -r;
        ...
        glm::mat4 model = Matrix_Identity(); // TransformaÃ§Ã£o identidade de modelagem

        // Enviamos as matrizes "view" e "projection" para a placa de vÃ­deo
        // (GPU). Veja o arquivo "shader_vertex.glsl", onde estas sÃ£o
        // efetivamente aplicadas em todos os pontos.
        glUniformMatrix4fv(g_view_uniform       , 1 , GL_FALSE , glm::value_ptr(view));
        glUniformMatrix4fv(g_projection_uniform , 1 , GL_FALSE , glm::value_ptr(projection));
        ...
        /*
        if (variaveis->estado == CORRE) {
            //Capivara vai da posiÃ§Ã£o Prev atÃ© a posiÃ§Ã£o Next
            variaveis->tempoDash = std::min(variaveis->tempoDash + delta_t, (float)MAXTEMPODASH);
            variaveis->capPos.x = variaveis->capPrevPos.x + (variaveis->capNextPos.x - variaveis->capPrevPos.x) * easing(variaveis->tempoDash/MAXTEMPODASH);
            variaveis->capPos.y = variaveis->capPrevPos.y + (variaveis->capNextPos.y - variaveis->capPrevPos.y) * easing(variaveis->tempoDash/MAXTEMPODASH);
        }
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    // Se o usuÃ¡rio apertar a tecla espaÃ§o, resetamos os Ã¢ngulos de Euler para zero.
